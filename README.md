...
5. Physical warehouse table (`database.schema.table`)
   - Emitted as dataset on mapped platform (for example Snowflake)
...
## Snowflake stitching guidance

To stitch Omni lineage to already-ingested Snowflake assets, table URNs must match exactly:

- `platform`
- `platform_instance` (if used in Snowflake source)
- `env`
